...
describe('Note Processor', () => {
  ...
  beforeEach(() => {
    ...
    keyStore.getMasterSecretKey.mockImplementation((pkM: PublicKey) => {
      if (pkM.equals(account.publicKeys.masterOutgoingViewingPublicKey)) {
  ...
  it('should store an outgoing note that belongs to us', async () => {
    const request = new MockNoteRequest(getRandomNoteLogPayload(app), 4, 0, 2, CompleteAddress.random().address, ownerOvKeys);
  ...
  it('should store multiple notes that belong to us', async () => {
    const requests = [
      ...
      new MockNoteRequest(getRandomNoteLogPayload(app), 9, 3, 2, CompleteAddress.random().address, KeyValidationRequest.random()),
  ...
  it('should not store notes that do not belong to us', async () => {
    ...
    const blocks = mockBlocks([
      new MockNoteRequest(getRandomNoteLogPayload(), 2, 1, 1, CompleteAddress.random().address, KeyValidationRequest.random()),
      new MockNoteRequest(getRandomNoteLogPayload(), 2, 3, 0, CompleteAddress.random().address, KeyValidationRequest.random()),
    ]);
    await noteProcessor.process(blocks);
